...
const handleAirTemps = async (specie) => {
  const query =
    "SELECT air_temperature FROM fish_catch_database.fish_catch join weather on fish_catch.weather_id = weather.weather_id join species on fish_catch.species_id = species.species_id where species_name = ?";
